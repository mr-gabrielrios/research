...
public  :: iceClocka,iceClockb,iceClockc
  ...
  character(len=128) :: version = '$Id: ice_type.F90,v 20.0.2.1 2014/02/04 17:21:29 Niki.Zadeh Exp $'
    ...
    if (do_icebergs) then
        if( ASSOCIATED(Ice%maskmap) ) then
          call icebergs_init(Ice%icebergs, &
             im, jm, layout, io_layout, Ice%axes(1:2), x_cyclic, tripolar_grid, &
             dt_slow, Time, geo_lonv_ib, geo_latv_ib, wett, dtn, dte, cell_area,&
             cos_rot, sin_rot, Ice%maskmap)
        else
          call icebergs_init(Ice%icebergs, &
             im, jm, layout, io_layout, Ice%axes(1:2),  x_cyclic, tripolar_grid, &
             dt_slow, Time, geo_lonv_ib, geo_latv_ib, wett, dtn, dte, cell_area, &

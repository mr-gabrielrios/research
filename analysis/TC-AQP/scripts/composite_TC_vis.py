...
def plot_TC_GCM_composite(entries_TC: xr.Dataset,
    ...
    for index, config_key in enumerate(configs.keys()):
        ...
        ax.pcolormesh(config_means[config_key].grid_xt_TC, config_means[config_key].grid_yt_TC, config_means[config_key], norm=norm, cmap=cmap)
        ...
        annotation.set_path_effects([matplotlib.patheffects.Stroke(linewidth=1, foreground='white'),
    ...
    for index, config_key in enumerate(configs.keys()):
        ...
        number_of_cax_ticks = int(np.floor(len(norm.boundaries) // 4))
        cax.xaxis.set_major_locator(matplotlib.ticker.MaxNLocator(nbins=number_of_cax_ticks))
        # Apply labeling
        long_name, units = visualization.field_properties(field_name)
        long_name = f'$\delta$ ({long_name})' if config_key == 'TC-GCM' else long_name

'''
Assigns GCM data to a given TC corresponding to the GCM run.
'''
...
def access_storm_tracks(model_name: str,
    ...
    print(f'[access_storm_tracks] {model_name}, {experiment_name}, {year_range}')
...
def test_single_storm(storm_track_data: pd.DataFrame):
    ...
    assert len(delta_longitude) > 0, f'delta_longitude = {delta_longitude}, but must be iterable.'
    if max(delta_longitude) >= threshold_delta_longitude:
        print(f'Longitude threshold exceeded for storm ID {storm_track_data['storm_id'].unique().item()}, value = {max(delta_longitude):.2f} degrees.')
        # assert max(delta_longitude) < threshold_delta_longitude
        criterion_flag = False
        
    delta_latitude = storm_track_data['center_lat'].diff().dropna().abs()
    assert len(delta_latitude) > 0, f'delta_latitude = {delta_latitude}, but must be iterable.'
    # assert max(delta_latitude) < threshold_delta_latitude
    if max(delta_latitude) >= threshold_delta_latitude:
        print(f'Latitude threshold exceeded, value = {max(delta_latitude):.2f} degrees.')
        criterion_flag = False
        
    # Ensure the necessary columns are in the storm DataFrame
    
    if 'cftime' not in storm_track_data.columns:
        print(f'cftime data is not in the track DataFrame.')
        # assert ('cftime' in storm_track_data.columns)
        criterion_flag = False
...
def pick_storm(track_data: pd.DataFrame,
    ...
    if test_single_storm(storm_track_data):

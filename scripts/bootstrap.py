...
def derived_field(model, field, frequency='month', resample=True):
    dirname = '/projects/GEOCLIM/gr7610/analysis/model_out'
    experiments = ['control', 'swishe']

    derived_fields = {'p-e': {'fields': ['precip', 'evap'],
                              'attrs': {'long_name': 'precipitation - evaporation',
                                        'units': 'mm d^-1'}},
                      'theta_e': {'fields': ['sphum', 'temp', 'rh'],
                                  'attrs': {'long_name': 'equivalent potential temperature',
                                            'units': 'K'}},
                      'cmf': {'fields': ['sphum', 'precip'],
                                  'attrs': {'long_name': 'Held-Soden convective mass flux',
                                            'units': ''}}}
    
    data = {}
    for experiment in experiments:
        data[experiment] = {}
        for subfield in derived_fields[field]['fields']:
            data[experiment][subfield] = None
            if 'swishe' in experiment:
                files = [os.path.join(dirname, file) for file in os.listdir(dirname) if
                         (model in file) & (subfield in file) & (frequency in file) & 
                         ('swishe' in file) & ('resample' in file) & (file.endswith('nc'))]
            else:
                files = [os.path.join(dirname, file) for file in os.listdir(dirname) if
                         (model in file) & (subfield in file) & (frequency in file) & 
                         ('swishe' not in file) & ('resample' in file) & (file.endswith('nc'))]
            print(files)
            data[experiment][subfield] = xr.open_dataset(files[0]).load().drop_duplicates('time')
        data[experiment] = xr.merge(data[experiment].values())

    for experiment in experiments:
        if field == 'cmf':
            r = data[experiment]['sphum']/(1 - data[experiment]['sphum']) # called q in Held and Soden (2006)
            data[experiment][field] = data[experiment]['precip']/r.sel(pfull=slice(850, 1000)).mean(dim='pfull')
        if field == 'p-e':
            data[experiment][field] = data[experiment]['precip'] - data[experiment]['evap']
        if field == 'theta_e':
            ...
            data[experiment][field] = (data[experiment]['temp']*(1000/data[experiment].pfull)**a) * (data[experiment]['rh']**b) * np.exp(L_v*r/(c_tot*data[experiment]['temp']))
            data[experiment][field] = data[experiment][field].where(data[experiment].pfull >= 85, np.nan)
        data[experiment][field].attrs = derived_fields[field]['attrs']
    
    return data

start_time = time.time()
model, field = 'HIRAM', 'temp'
